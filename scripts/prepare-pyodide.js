...
async function initNetworkProxyFromEnv() {
	...
	try {
		...
		console.warn('undici package is not available. Skipping proxy initialization.');
		return;
	}
...
async function downloadPackages() {
	...
	let loadPyodide;
	try {
		// Use dynamic import so build can continue even when pyodide package
		// is not installed in constrained/offline build environments.
		({ loadPyodide } = await import('pyodide'));
	} catch (err) {
		console.warn(
			'Pyodide package is not available. Skipping package download and using existing static assets if present.'
		);
		return;
	}
...
async function copyPyodide() {
	...
	try {
		await access('node_modules/pyodide');
	} catch {
		console.warn(
			'node_modules/pyodide not found. Skipping static pyodide copy to avoid build failure.'
		);
		return;
	}
