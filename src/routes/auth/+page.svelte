<script lang="ts">
	...
	let onboarding = false;
		...
		const logo = document.getElementById('logo');
			...
			if (logo) {
				logo.src = `${WEBUI_BASE_URL}/static/wizeai.png`;
				logo.style.filter = '';
			}
		}
...
<div class="w-full h-screen max-h-[100dvh] text-white relative" id="auth-page">
	<div
		...
		style="background-image: url('{WEBUI_BASE_URL}/static/backscreen.png')"
	></div>
	<div class="w-full h-full absolute top-0 left-0 bg-black/55"></div>

	<div class="w-full absolute top-0 left-0 right-0 h-8 drag-region" />
	<div class="absolute top-4 right-4 z-50 no-drag-region">
		<a
			href="https://protectwize.com"
			aria-label="Visit ProtectWize"
		>
			<img src="{WEBUI_BASE_URL}/static/protectwize.png" class="h-8 w-auto" alt="ProtectWize" />
		</a>
	</div>
							...
							{#if $config?.metadata?.auth_logo_position === 'center'}
								<div class="flex justify-center mb-6">
										<img
											...
											src="{WEBUI_BASE_URL}/static/wizeai.png"
								...
								<div class="mt-5">
									{#if $config?.features.enable_login_form || $config?.features.enable_ldap || form}
										...
										{:else}
											...
											{#if canCreateUserAccount() && mode === 'signin'}
												<button
													class="mt-2 bg-transparent border border-gray-400/40 hover:border-gray-300/70 hover:bg-white/5 transition w-full rounded-full font-medium text-sm py-2.5"
													type="button"
													on:click={() => {
											...
											{#if canCreateUserAccount() && mode === 'signup'}
