...
https://github.com/wize-ai/wize-ai
"""
)
...
app.add_middleware(
    ...
    # Browser cookies/auth headers must not be allowed with wildcard origins.
        ...
        **({"onboarding": True} if onboarding else {}),
        ...
        "features": {
            ...
            "enable_oauth_signup": bool(
                ENABLE_OAUTH_SIGNUP or app.state.config.ENABLE_SIGNUP
            ),
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(
                "https://api.github.com/repos/wize-ai/wize-ai/releases/latest",
...
async def get_current_usage(user=Depends(get_verified_user)):
    """
    Get current usage statistics for WizeAI.
...
async def get_manifest_json():
    if app.state.EXTERNAL_PWA_MANIFEST_URL:
        ...
        return {
            ...
            "description": f"{app.state.WEBUI_NAME} by ProtectWize is an open, extensible, user-friendly interface for AI that adapts to your workflow.",
            "start_url": "/",
            "display": "standalone",
            "background_color": "#343541",
            "icons": [
                {
                    "src": "/static/wizeai.png",
                    "type": "image/png",
                    "sizes": "500x500",
                    "purpose": "any",
                },
...
async def get_opensearch_xml():
    ...
    <Image width="16" height="16" type="image/x-icon">{app.state.config.WEBUI_URL}/static/wizeai.png</Image>
...
async def serve_cache_file(
    ...
    file_path = (cache_dir / path).resolve()
    # Enforce strict directory containment to prevent traversal/sibling-prefix bypasses.
    try:
