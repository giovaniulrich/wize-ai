        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                with zipfile.ZipFile(tmp_zip_path, "r") as zip_ref:
                    ...
                    # Bound extraction complexity to reduce zip bomb risk.
                    ...
                    if total_uncompressed > MAX_ZIP_UNCOMPRESSED_BYTES:
                        raise HTTPException(
                            status.HTTP_400_BAD_REQUEST,
                            detail="ZIP file is too large after extraction",
                        )

                    for member in members:
                        member_name = member.filename

                        # Reject absolute and parent-directory paths before extraction.
                        if os.path.isabs(member_name):
                            raise HTTPException(
                                status.HTTP_400_BAD_REQUEST,
                                detail="ZIP contains invalid absolute paths",
                            )

                        normalized_member = Path(member_name)
                        if ".." in normalized_member.parts:
                            raise HTTPException(
                                status.HTTP_400_BAD_REQUEST,
                                detail="ZIP contains path traversal entries",
                            )

                        dest_path = (tmp_dir_path / normalized_member).resolve()
                        try:
                            dest_path.relative_to(tmp_dir_path)
                        except ValueError:
                            raise HTTPException(
                                status.HTTP_400_BAD_REQUEST,
                                detail="ZIP extraction attempted outside target directory",
                            )

                        # Reject symbolic links in archives.
                        file_type_bits = (member.external_attr >> 16) & 0o170000
                        if file_type_bits == 0o120000:
                            raise HTTPException(
                                status.HTTP_400_BAD_REQUEST,
                                detail="ZIP contains symbolic links, which are not allowed",
                            )

                        zip_ref.extract(member, tmp_dir_path)
