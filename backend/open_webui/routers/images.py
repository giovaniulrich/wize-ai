...
async def image_edits(
    ...
    try:
        ...
        async def load_url_image(data):
            ...
            if data.startswith("http://") or data.startswith("https://"):
                # Disallow remote URL fetching from user-controlled input to prevent SSRF.
                raise HTTPException(
                    status_code=400,
                    detail=ERROR_MESSAGES.DEFAULT(
                        "Remote image URLs are not allowed for image edits. Upload the image first or use a data URL."
                    ),
                )
